...
CFunctionalPotential::Create(CModelDomain* pMD, const intVector& domain,
        int inMoment,
...
CFunctionalPotential*
CFunctionalPotential::Create( const intVector& domain,
        CModelDomain* pMD,
        int inMoment,
...
CFunctionalPotential*
CFunctionalPotential::CreateDeltaFunction(const intVector& domain,
        CModelDomain* pMD,
...
CFunctionalPotential*
CFunctionalPotential::CreateUnitFunctionDistribution(
...
CFunctionalPotential*
CFunctionalPotential::Copy(const CFunctionalPotential *pGauPot)
{
    PNL_CHECK_IS_NULL_POINTER( pGauPot );

    CFunctionalPotential *retPot = new CFunctionalPotential( *pGauPot );
    PNL_CHECK_IF_MEMORY_ALLOCATED( retPot );
    return retPot;
}

CFunctionalPotential*
CFunctionalPotential::Create( const int *domain, int nNodes,
        CModelDomain* pMD,
        ...
        const intVector& obsIndices )
{
    ...
    CFunctionalPotential *pNewParam = new CFunctionalPotential(domain, nNodes,
            pMD, obsIndices);
...
CFunctionalPotential*
CFunctionalPotential::CreateDeltaFunction( const int *domain,
        ...
        const intVector& obsIndices )
{
    ...
    CFunctionalPotential *resPot = CFunctionalPotential::Create( domain, nNodes,
            pMD, isInMoment, NULL, NULL, 0.0f, obsIndices );
    const pConstNodeTypeVector* ntVec = resPot->GetArgType();
    CFunctionalDistribFun *DeltaData =
    CFunctionalDistribFun::CreateDeltaDistribution( nNodes, &ntVec->front(),
            pMean, isInMoment );
...
CFunctionalPotential* CFunctionalPotential::CreateUnitFunctionDistribution(
        ...
        const intVector& obsIndices )
{
    ...
    const pConstNodeTypeVector* ntVec = resPot->GetArgType();
    CFunctionalDistribFun* UniData =
    CFunctionalDistribFun::CreateUnitFunctionDistribution( nNodes,
...
CFactor* CFunctionalPotential::CloneWithSharedMatrices()
{
    CFunctionalPotential* resPot = new CFunctionalPotential(this);
    PNL_CHECK_IF_MEMORY_ALLOCATED(resPot);

    return resPot;
}

CFactor* CFunctionalPotential::Clone() const
{
    const CFunctionalPotential* self = this;
    CFunctionalPotential* res = CFunctionalPotential::Copy(self);
    return res;
}

CFunctionalPotential::CFunctionalPotential( const CFunctionalPotential &pGauPot )
        ...
        pGauPot.m_obsPositions )
{
    ...
    m_CorrespDistribFun = CFunctionalDistribFun::Copy(
            static_cast<CFunctionalDistribFun*>(pGauPot.m_CorrespDistribFun));
    m_Domain = intVector( pGauPot.m_Domain );
}

CFunctionalPotential:: CFunctionalPotential(const int *domain, int nNodes,
        CModelDomain* pMD,
        const intVector& obsIndices ):
...
CFunctionalPotential::CFunctionalPotential(const CFunctionalPotential* pGauPot):CPotential(pGauPot)
{
}

float CFunctionalPotential::ProcessingStatisticalData( int numberOfEvidences)
{
    return 1.0f;
}

void CFunctionalPotential::UpdateStatisticsEM( const CPotential *pMargPot,
...
void CFunctionalPotential::UpdateStatisticsML(const pConstEvidenceVector& evidencesIn)
{
    UpdateStatisticsML( &evidencesIn.front(), evidencesIn.size() );
}

void CFunctionalPotential::UpdateStatisticsML(const CEvidence* const* pEvidences,
        int EvidenceNumber)
{
    if( !pEvidences )
    {
...
void CFunctionalPotential::Dump() const
{
    int i;
...
void CFunctionalPotential::SetCoefficient( float coeff, int isForCanonical )
{
    ...
    {
        static_cast<CFunctionalDistribFun*>(m_CorrespDistribFun)->SetCoefficient( coeff,
                isForCanonical );
    }
}

float CFunctionalPotential::GetCoefficient( int forCanonical )
{
    ...
    {
        return static_cast<CFunctionalDistribFun*>(m_CorrespDistribFun)->GetCoefficient(
                forCanonical );
    }
}

void CFunctionalPotential::
...
{   
    ...
    if( nonObsNodes.size() )
    {
        ...
        if(int(nonObsNodes.size()) != nnodes)
        {
            ...
            static_cast<CFunctionalDistribFun*>(pMargPot->GetDistribFun())->UpdateMomentForm();
        ...
        {
            static_cast<CFunctionalDistribFun*>(this->GetDistribFun())->UpdateMomentForm();
...
CPotential* CFunctionalPotential::ConvertStatisticToPot(int numOfSamples) const
{   
    ...
    CFunctionalPotential *pPot = CFunctionalPotential::
...
float CFunctionalPotential::GetLogLik( const CEvidence* pEv, const CPotential* pShrInfRes ) const
{   
    ...
    if(!pShrInfRes )
    {
        ...
        float logLik = float( static_cast<CFunctionalDistribFun*>(GetDistribFun())->
...
#ifdef PAR_PNL
void CFunctionalPotential::UpdateStatisticsML(CFactor *pPot)
{
    //Is pPot correct?
    if (pPot->GetDistributionType() != dtGaussian)
    PNL_THROW(CInconsistentType,
            "Can not use function CFunctionalPotential::UpdateStatisticsML with wrong distribution type");
...
#endif // PAR_OMP
#ifdef PNL_RTTI
const CPNLType CFunctionalPotential::m_TypeInfo = CPNLType("CFunctionalPotential", &(CPotential::m_TypeInfo));
