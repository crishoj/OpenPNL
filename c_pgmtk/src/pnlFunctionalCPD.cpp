...
CFunctionalCPD* CFunctionalCPD::Create(const intVector& domain,
    CModelDomain* pMD)
{
    return CFunctionalCPD::Create( &domain.front(), domain.size(), pMD);
}
CFunctionalCPD* CFunctionalCPD::CreateUnitFunctionCPD(const intVector& domain,
    CModelDomain* pMD)
{
    return CFunctionalCPD::CreateUnitFunctionCPD( &domain.front(),
                                                 domain.size(), pMD);
}

void CFunctionalCPD::AllocDistribution(
    const C2DNumericDenseMatrix<float>* meanMat,
    const C2DNumericDenseMatrix<float>* covMat, float normCoeff,
    const p2DDenseMatrixVector& weightsMat, const intVector& parentCombination)
{
    int nel;
    const float * vls;
...
void CFunctionalCPD::AllocDistribution(const floatVector& mean,
    const floatVector& cov, float normCoeff, const floatVecVector& weights,
...
void CFunctionalCPD::SetCoefficientVec(float coeff,
    const intVector& parentCombination)
{
    SetCoefficient(coeff, &parentCombination.front() );
}
float CFunctionalCPD::GetCoefficientVec(const intVector& parentCombination)
{
    return GetCoefficient( &parentCombination.front() );
}

CFunctionalCPD* CFunctionalCPD::Create(const int *domain, int nNodes,
    CModelDomain* pMD)
{
    ...
    CFunctionalCPD *pNewParam = new CFunctionalCPD( domain, nNodes, pMD);
    PNL_CHECK_IF_MEMORY_ALLOCATED( pNewParam );
    return pNewParam;
}

CFunctionalCPD* CFunctionalCPD::Copy(const CFunctionalCPD* pGauCPD)
{
    PNL_CHECK_IS_NULL_POINTER( pGauCPD );
    
    CFunctionalCPD *retCPD = new CFunctionalCPD( *pGauCPD );
    PNL_CHECK_IF_MEMORY_ALLOCATED( retCPD );
    return retCPD;
}

CFunctionalCPD* CFunctionalCPD::CreateUnitFunctionCPD(const int *domain,
    int nNodes, CModelDomain* pMD)
{
    ...
    CFunctionalCPD* resCPD = new CFunctionalCPD( domain, nNodes, pMD );
...
CFactor* CFunctionalCPD::CloneWithSharedMatrices()
{
    CFunctionalCPD* resCPD = new CFunctionalCPD(this);
    PNL_CHECK_IF_MEMORY_ALLOCATED(resCPD);
    
    return resCPD;
}

CFactor* CFunctionalCPD::Clone() const
{
    const CFunctionalCPD* self = this;
    CFunctionalCPD* res = CFunctionalCPD::Copy(self);
    return res;
}

CFunctionalCPD::CFunctionalCPD(const CFunctionalCPD& GauCPD) :
    CCPD(dtGaussian, ftCPD, GauCPD.GetModelDomain() )
{
    ...
    if (GauCPD.m_CorrespDistribFun->GetDistributionType() == dtGaussian)
    {
        ...
        m_CorrespDistribFun
                = CFunctionalDistribFun::Copy( static_cast<CFunctionalDistribFun*>(GauCPD.m_CorrespDistribFun ));
    } else
    {
        if (GauCPD.m_CorrespDistribFun->GetDistributionType() == dtCondGaussian)
        {
            delete m_CorrespDistribFun;
            m_CorrespDistribFun
                    = CCondFunctionalDistribFun::Copy( static_cast<CCondFunctionalDistribFun*>(GauCPD.m_CorrespDistribFun));
        } else
        {
...
CFunctionalCPD::CFunctionalCPD(const int *domain, int nNodes, CModelDomain* pMD) :
    CCPD(dtGaussian, ftCPD, domain, nNodes, pMD)
{
}

CFunctionalCPD::CFunctionalCPD(const CFunctionalCPD* pGauCPD) :
    CCPD(pGauCPD)
{
}

void CFunctionalCPD::AllocDistribution(const float* pMean, const float* pCov,
    float normCoeff, const float* const* pWeights, const int* parentCombination)
{
    ...
    if (m_CorrespDistribFun->GetDistributionType() == dtGaussian)
    {
        ...
        if (pWeights)
        {
            ...
            for (int i = 0; i < numParents; i++)
            {
                if (pWeights[i])
                {
                    m_CorrespDistribFun->AllocMatrix(pWeights[i], matWeights, i);
                }
            }
        }
    } else
    {
        ...
        if (pWeights)
        {
            ...
            static_cast<CCondFunctionalDistribFun*>(m_CorrespDistribFun)->
            ...
            for (int i = 0; i < numParents; i++)
            {
                if (pWeights[i])
                {
                    m_CorrespDistribFun->AllocMatrix(pWeights[i], matWeights,
                                                     i, parentCombination);
                }
            }
        }
...
void CFunctionalCPD::NormalizeCPD()
{
    CDistribFun *normData = m_CorrespDistribFun->GetNormalized();
    SetDistribFun(normData);
}

CPotential *CFunctionalCPD::ConvertToPotential() const
{
    CDistribFun *gauFactData = (m_CorrespDistribFun)->ConvertCPDDistribFunToPot();
    CFunctionalPotential *resFactor =
            CFunctionalPotential::Create( &m_Domain.front(), m_Domain.size(),
                                         GetModelDomain() );
...
CPotential* CFunctionalCPD::ConvertWithEvidenceToPotential(
    const CEvidence* pEvidence, int flagSumOnMixtureNode) const
{
    ...
    } else //it means m_CorrespDistribFun->GetDistributionType == dtCondGaussian
    {
        ...
        if ( (obsDiscreteIndex.size() + obsGauIndex.size() == m_Domain.size()-1)
                && isLastNodeObs)
        {
            //result distribution is scalar
            obsDiscreteIndex.insert(obsDiscreteIndex.end(),
                                    obsGauIndex.begin(), obsGauIndex.end());
            //child node is observed
            obsDiscreteIndex.insert(obsDiscreteIndex.end(), domSize-1);
            resPot = CScalarPotential::Create(m_Domain, GetModelDomain(),
                                              obsDiscreteIndex);
        }
        ...
        else
        {
            ...
            {
                CCondFunctionalDistribFun* withDiscrEv = (static_cast<CCondFunctionalDistribFun*>(m_CorrespDistribFun))->
                ...
                //child node is observed
                obsDiscreteIndex.insert(obsDiscreteIndex.end(), domSize-1);
                resPot = CTabularPotential::Create( &m_Domain.front(),
                                                   m_Domain.size(),
                                                   GetModelDomain(), NULL,
                                                   obsDiscreteIndex);
            ...
            } else
            {
                if (allDiscrObs && !allContObs)
                {
                    ...
                    {
                        location = std::find(pObsNodes.begin(),
                                             pObsNodes.end(), discParents[j])
                                - pObsNodes.begin();
                        parentComb[j] = pObsValues[location]->GetInt();
                    }
                    
                    const CFunctionalDistribFun
                            * resDistr =
                                    static_cast<CCondFunctionalDistribFun*>(m_CorrespDistribFun)->GetDistribution(parentComb);
                    
                    CDistribFun* newResDistr =
                            resDistr->ConvertCPDDistribFunToPot();
                    
                    obsGauIndex.insert(obsGauIndex.end(),
                                       obsDiscreteIndex.begin(),
...
void CFunctionalCPD::SetCoefficient(float coeff, const int* pParentCombination)
{
...
float CFunctionalCPD::GetCoefficient(const int* parentCombination)
{
    EDistributionType dt = m_CorrespDistribFun->GetDistributionType();
    int forCanonical = 0;
    float val= FLT_MAX;
    ...
    } else
    {
        if (dt == dtCondGaussian)
        {
            val = static_cast<CCondFunctionalDistribFun*>(m_CorrespDistribFun)
...
void CFunctionalCPD::UpdateStatisticsEM(const CPotential *pMargPot,
    const CEvidence *pEvidence)
{
    if ( !pMargPot)
    {
        PNL_THROW( CNULLPointer, "evidences" )
        //no corresp evidences
    }
    ...
    if (obsPos.size() && (this->GetDistribFun()->GetDistributionType() != dtCondGaussian))
    {
        ...
        m_CorrespDistribFun->UpdateStatisticsEM(pExpandPot->GetDistribFun(),
                                                pEvidence, 1.0f,
                                                 &m_Domain.front());
        delete pExpandPot;
    } else
    {
        m_CorrespDistribFun->UpdateStatisticsEM(pMargPot->GetDistribFun(),
                                                pEvidence, 1.0f,
                                                 &m_Domain.front() );
    }
    
}
float CFunctionalCPD::ProcessingStatisticalData(int numberOfEvidences)
{
    return m_CorrespDistribFun->ProcessingStatisticalData(static_cast<float>(numberOfEvidences));
}

void CFunctionalCPD::UpdateStatisticsML(const CEvidence* const* pEvidences,
    int EvidenceNumber)
{
    if ( !pEvidences)
    {
...
void CFunctionalCPD::GenerateSample(CEvidence* evidence, int maximize) const
{
    int nnodes;
    const int *domain;
    ...
    if (this->GetDistribFun()->GetDistributionType() == dtCondGaussian)
    {
        ...
        const CFunctionalDistribFun *currentDistrib =
                tmpDistr->GetDistribution(&discrParentVls.front());
        ...
        CFunctionalCPD *tmpCPD = CFunctionalCPD::Create(domain,
                                                        this->GetModelDomain());
...
CPotential* CFunctionalCPD::ConvertStatisticToPot(int numOfSamples) const
{
    
    PNL_CHECK_LEFT_BORDER( numOfSamples, 0);
    
    C2DNumericDenseMatrix<float>* pLearnMatrixMean =
            static_cast<C2DNumericDenseMatrix<float> *> (this->GetDistribFun()->GetStatisticalMatrix(stMatMu));
    C2DNumericDenseMatrix<float>* pLearnMatrixCov =
            static_cast<C2DNumericDenseMatrix<float> *> (this->GetDistribFun()->GetStatisticalMatrix(stMatSigma));
    ...
    C2DNumericDenseMatrix<float> *pMatrixMean =
            static_cast<C2DNumericDenseMatrix<float> *> (this->GetDistribFun()->GetMatrix(matMean));
    C2DNumericDenseMatrix<float> *pMatrixCov =
            static_cast<C2DNumericDenseMatrix<float> *> (this->GetDistribFun()->GetMatrix(matCovariance));
    ...
    C2DNumericDenseMatrix<float>* meanSquare = pnlMultiply(pMatrixMean,
                                                           matrixMeanTr, 0);
...
float CFunctionalCPD::GetLogLik(const CEvidence* pEv,
    const CPotential* pShrInfRes) const
{
    ...
    if (!pShrInfRes)
    {
        ...
        const CFunctionalDistribFun *currentDistrib= NULL;
        if (this->GetDistribFun()->GetDistributionType() == dtCondGaussian)
        {
            const CCondFunctionalDistribFun
                    *tmpDistr =
                            (static_cast<CCondFunctionalDistribFun*> (this->GetDistribFun()));
        ...
        {
            currentDistrib
                    = static_cast<const CFunctionalDistribFun*>(GetDistribFun());
            contDomain = domain;
        }
        
        intVector ndsSz(contDomain.size());
        
        std::vector<C2DNumericDenseMatrix<float> *> pValMat(contDomain.size(), 
        NULL);
        std::vector<C2DNumericDenseMatrix<float>*>::iterator pMatIt=
                pValMat.begin();
        ...
        for (; it != contDomain.end(); it++, pMatIt++, i++)
        {
            ...
            if (pEv->IsNodeObserved(*it) )
            {
                ...
                int sizes[] = { ndSz, 1 };
                *pMatIt = C2DNumericDenseMatrix<float>::Create(sizes,
                                                                &data.front());
            } else
            {
...
#ifdef PAR_PNL
void CFunctionalCPD::UpdateStatisticsML(CFactor *pPot)
{
    PNL_THROW(CNotImplemented,
            "UpdateStatisticsML for CFunctionalCPD not implemented yet");
};
#endif // PAR_OMP
#ifdef PNL_RTTI
const CPNLType CFunctionalCPD::m_TypeInfo = CPNLType("CFunctionalCPD", &(CCPD::m_TypeInfo));
