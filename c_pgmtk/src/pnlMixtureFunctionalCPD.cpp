...
CMixtureFunctionalCPD* CMixtureFunctionalCPD::Create(const intVector& domain,
    CModelDomain* pMD, const floatVector& probabilities)
{
    return CMixtureFunctionalCPD::Create( &domain.front(), domain.size(), pMD,
                                         &probabilities.front() );
}

void CMixtureFunctionalCPD::AllocDistributionVec(
    const C2DNumericDenseMatrix<float>* meanMat,
    const C2DNumericDenseMatrix<float>* covMat, float normCoeff,
    const p2DDenseMatrixVector& weightsMat, const intVector& discrComb)
{
    int nel;
    const float * vls;
...
void CMixtureFunctionalCPD::AllocDistributionVec(const floatVector& mean,
...
CMixtureFunctionalCPD* CMixtureFunctionalCPD::Create(const int *domain,
    int nNodes, CModelDomain* pMD, const float* probabilities)
{
    ...
    } else
    {
        ...
        CMixtureFunctionalCPD *pNewParam = new CMixtureFunctionalCPD( domain,
...
CMixtureFunctionalCPD* CMixtureFunctionalCPD::Copy(
    const CMixtureFunctionalCPD* pGauCPD)
{
    PNL_CHECK_IS_NULL_POINTER( pGauCPD );
    
    CMixtureFunctionalCPD *retCPD = new CMixtureFunctionalCPD( *pGauCPD );
    PNL_CHECK_IF_MEMORY_ALLOCATED( retCPD );
    return retCPD;
}

CFactor* CMixtureFunctionalCPD::CloneWithSharedMatrices()
{
    CMixtureFunctionalCPD* resCPD = new CMixtureFunctionalCPD(this);
    PNL_CHECK_IF_MEMORY_ALLOCATED(resCPD);
    
    return resCPD;
}

CFactor* CMixtureFunctionalCPD::Clone() const
{
    const CMixtureFunctionalCPD* self = this;
    CMixtureFunctionalCPD* res = CMixtureFunctionalCPD::Copy(self);
    return res;
}

CMixtureFunctionalCPD::CMixtureFunctionalCPD(const CMixtureFunctionalCPD& GauCPD) :
...
CMixtureFunctionalCPD::CMixtureFunctionalCPD(const int *domain, int nNodes,
...
CMixtureFunctionalCPD::CMixtureFunctionalCPD(
    const CMixtureFunctionalCPD* pMixCPD) :
    CCPD(pMixCPD)
{
}

void CMixtureFunctionalCPD::AllocDistribution(const float* pMean,
    const float* pCov, float normCoeff, const float* const* pWeights,
    const int* discrComb)
{
...
void CMixtureFunctionalCPD::NormalizeCPD()
{
...
CPotential *CMixtureFunctionalCPD::ConvertToPotential() const
{
...
CPotential* CMixtureFunctionalCPD::ConvertWithEvidenceToPotential(
    const CEvidence* pEvidence, int flagSumOnMixtureNode) const
{
    ...
    CCondFunctionalDistribFun* selfDistr =
            static_cast<CCondFunctionalDistribFun*> (m_CorrespDistribFun);
    ...
    for (i = 0; i < domSize; i++)
    {
        if ( (location = std::find(itObsNodesBeg, itObsNodesEnd, m_Domain[i]) )
                != itObsNodesEnd)
        {
    ...
    const pConstNodeTypeVector* ntFromCPD =
    ...
    EnterDiscreteEvidence(obsDiscreteIndex.size(), &obsDiscreteIndex.front(),
                          &obsDiscrVals.front(), pObsTabNodeType);
    //need to enter continuous evidence
    CTabularDistribFun* resDistr = withDiscrEv->
    EnterFullContinuousEvidence(obsGauIndex.size(), &obsGauIndex.front(),
                                obsSelfVal, &obsGauVals.front(),
                                pObsGauNodeType);
    delete obsSelfVal;
...
void CMixtureFunctionalCPD::SetCoefficientVec(float coeff,
    const intVector& parentCombination)
{
    SetCoefficient(coeff, &parentCombination.front() );
}

float CMixtureFunctionalCPD::GetCoefficientVec(
    const intVector& parentCombination)
{
    return GetCoefficient( &parentCombination.front() );
}

void CMixtureFunctionalCPD::SetCoefficient(float coeff, const int* discrComb)
{
...
float CMixtureFunctionalCPD::GetCoefficient(const int* discrComb)
{
    int forCanonical = 0;
    return static_cast<CCondFunctionalDistribFun*>(m_CorrespDistribFun)
    ->GetCoefficient(forCanonical, discrComb);
}

#if 0
void CMixtureFunctionalCPD::UpdateStatisticsEM( const CPotential *pMargPot,
        const CEvidence *pEvidence )
{
    ...
    intVector discrIndices;
    intVector contIndices;
    CCondFunctionalDistribFun* correspDistr =
    static_cast<CCondFunctionalDistribFun*>(m_CorrespDistribFun);
    ...
    const CEvidence *pEvidence)
{
    ...
    int discSize = discrIndices.size();
    int mixNodePos = discSize - 1;
    CMatrix<float>* shrMat = mat->ReduceOp( &discrIndices[mixNodePos], 1, 0);
    
    int mClass = shrMat->GetMatrixClass();
    if (!((mClass == mcSparse )||(mClass == mcNumericSparse)))
    {
        CDenseMatrix<float>* denseMat =
                static_cast<CDenseMatrix<float>*>(shrMat);
        const floatVector* vec = denseMat->GetVector();
        int sizeLearned = vec->size();
        int sizeProbHere = m_learnProbabilities.size();
        if (sizeLearned != sizeProbHere)
...
#endif
float CMixtureFunctionalCPD::ProcessingStatisticalData(int numberOfEvidences)
{
...
void CMixtureFunctionalCPD::UpdateStatisticsML(
    const CEvidence* const* pEvidences, int EvidenceNumber)
{
    if ( !pEvidences)
    {
...
int CMixtureFunctionalCPD::GetNumberOfMixtureNode() const
{
...
void CMixtureFunctionalCPD::GenerateSample(CEvidence* evidence, int maximize) const
{
    if (maximize)
    {
        PNL_THROW(CNotImplemented, "not implemented for mixture gaussin")
    }
    const CCondFunctionalDistribFun *pDistrFun =
            static_cast<const CCondFunctionalDistribFun*>(m_CorrespDistribFun);
    ...
    const CFunctionalDistribFun *pCurrentDistr;
    pCurrentDistr = pDistrFun->GetDistribution( &valuesVec.front() );
    
    CFunctionalCPD *pFunctionalCPD;
    pFunctionalCPD = CFunctionalCPD::Create(contNodes, this->GetModelDomain() );
    pFunctionalCPD->SetDistribFun(pCurrentDistr);
    
    pFunctionalCPD->GenerateSample(evidence, maximize);
    delete pFunctionalCPD;
}

CPotential* CMixtureFunctionalCPD::ConvertStatisticToPot(int numOfSamples) const
{
    return NULL;
}

float CMixtureFunctionalCPD::GetLogLik(const CEvidence* pEv,
    const CPotential* pShrInfRes) const
{
    ...
    CModelDomain* pMd = GetModelDomain();
    CFunctionalCPD *tmpCPD = CFunctionalCPD::Create(domain, pMd);
    const CCondFunctionalDistribFun *pDistrFun =
            static_cast<const CCondFunctionalDistribFun*>(GetDistribFun());
...
#ifdef PAR_PNL
void CMixtureFunctionalCPD::UpdateStatisticsML(CFactor *pPot)
{
    PNL_THROW(CNotImplemented,
            "UpdateStatisticsML for CMixtureFunctionalCPD not implemented yet");
};
#endif // PAR_OMP
#ifdef PNL_RTTI
const CPNLType CMixtureFunctionalCPD::m_TypeInfo = CPNLType("CMixtureFunctionalCPD", &(CCPD::m_TypeInfo));
